...
//all get from irrlicht 1.6, need to remove something don't want and use
...
#  warn IRRLICHT_FAST_MATH not supported.
	...
	inline f32 interpEaseInOut( f32 valueFrom, f32 valueTo, f32 alpha, f32 exp )
	{
		f32 modifiedAlpha;

		if( alpha < 0.5f )
		{
			modifiedAlpha = 0.5f * powf(2.f * alpha, exp);
		}
		else
		{
			modifiedAlpha = 1.f - 0.5f * powf(2.f * (1.f - alpha), exp);
		}

		return lerp<f32>(valueFrom, valueTo, modifiedAlpha);
	}

	inline f32 interpEaseIn(f32 valueFrom, f32 valueTo, f32 alpha, f32 exp)
	{
		return lerp<f32>(valueFrom, valueTo, powf(alpha,exp));
	}

	inline f32 interpEaseOut(f32 valueFrom, f32 valueTo, f32 alpha, f32 exp)
	{
		return lerp<f32>(valueFrom, valueTo, powf(alpha,1/exp));
	}
