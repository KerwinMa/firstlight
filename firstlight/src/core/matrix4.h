...
namespace core
{
	...
	class CMatrix4
	{
		public:
            ...
            CMatrix4<T>& setRotationDegrees2D(float rotX, float rotY);
            CMatrix4<T>& setRotationRadians2D(float rotX, float rotY);  
			...
			//! Builds a OpenGL convention perspective projection matrix based on a field of view
			CMatrix4<T>& buildProjectionMatrixPerspectiveFovOpenGL(f32 fieldOfViewRadians, f32 aspectRatio, f32 zNear, f32 zFar);
			...
			//! Builds a opengl convention orthogonal projection matrix.
			CMatrix4<T>& buildProjectionMatrixOrthoOpenGL(f32 widthOfViewVolume, f32 heightOfViewVolume, f32 zNear, f32 zFar);
	...
	*/
    ...
    template<class T>
    inline CMatrix4<T>& CMatrix4<T>::setRotationDegrees2D(float rotX, float rotY)
    {
        return setRotationRadians2D( rotX * core::DEGTORAD, rotY * core::DEGTORAD );
    }
    
    template<class T>
    inline CMatrix4<T>& CMatrix4<T>::setRotationRadians2D(float rotX, float rotY)
    {
        M[0] = cos(-rotX);
        M[1] = sin(-rotY);
        M[4] = -sin(-rotX);
	...
	*/
	...
	// Builds a OpenGL convention perspective projection matrix based on a field of view
	template <class T>
	inline CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixPerspectiveFovOpenGL(
			f32 fieldOfViewRadians, f32 aspectRatio, f32 zNear, f32 zFar)
	{
		const f64 h = reciprocal(tan(fieldOfViewRadians*0.5));
		FLT_DEBUG_BREAK_IF(aspectRatio==0.f); //divide by zero
		const T w = (T)(h / aspectRatio);		

		FLT_DEBUG_BREAK_IF(zNear==zFar); //divide by zero
		M[0] = w;
		M[1] = 0;
		M[2] = 0;
		M[3] = 0;
...
//		M[14] = (T)(zNear*zFar/(zNear-zFar)); // DirectX version
	...
	{
		FLT_DEBUG_BREAK_IF(widthOfViewVolume==0.f); //divide by zero
		FLT_DEBUG_BREAK_IF(heightOfViewVolume==0.f); //divide by zero
		FLT_DEBUG_BREAK_IF(zNear==zFar); //divide by zero
	...
	// Builds a opengl convention orthogonal projection matrix.
	template <class T>
	inline CMatrix4<T>& CMatrix4<T>::buildProjectionMatrixOrthoOpenGL(
			f32 widthOfViewVolume, f32 heightOfViewVolume, f32 zNear, f32 zFar)
	{
		FLT_DEBUG_BREAK_IF(widthOfViewVolume==0.f); //divide by zero
		FLT_DEBUG_BREAK_IF(heightOfViewVolume==0.f); //divide by zero
		...
		M[0] = (T)(2.0f/widthOfViewVolume);
		M[1] = 0;
		M[2] = 0;
		M[3] = 0;

		M[4] = 0;
		M[5] = (T)(2.0f/heightOfViewVolume);
		M[6] = 0;
		M[7] = 0;

		M[8] = 0;
		M[9] = 0;
		M[10] = (T)(2.0f/(zNear-zFar));
		M[11] = 0;
